...
class SpeechToTextApp(tk.Tk):
    ...
    def suggest_sections(self,ans,ds,question_number,min_suggestions=1):
        ...
        section_embedding=model.encode(ds['ans1'])
        similarities=util.pytorch_cos_sim(ans_embedding,section_embedding)[0]
        suggestion=None
        if question_number>=0 and question_number<len(ds['question']):
            suggestion={
                'index':question_number,
                'question':ds['question'][question_number],
                'ans':ds['ans'][question_number],
    ...
    def convert_audio_to_text(self):
        ...
        try:
            with sr.AudioFile(self.audio_file_path) as source:
                ...
                ds=pickle.load(open('preprocess_data.pkl','rb'))
                if 0<=question_number<len(ds['question']):
                    ...
                    if suggestion:
                        ...
                        self.output_text.insert(tk.END,"No relevant suggestion found.\n")
                else:
                    self.output_text.insert(tk.END,f"Invalid question number. Please enter a number between 1 and {len(ds['question'])}.\n")
        except sr.UnknownValueError:
            messagebox.showwarning("Speech to Text","Could not understand the audio")
        except sr.RequestError as e:
            messagebox.showwarning("Speech to Text",f"Error occurred: {e}")
        except ValueError:
            messagebox.showwarning("Input Error","Please enter a valid question number")
        except FileNotFoundError:
            messagebox.showwarning("File Error","No audio file found. Please record audio first.")
