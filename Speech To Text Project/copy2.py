...
class SpeechToTextApp(tk.Tk):
    def __init__(self):
        ...
        self.stop_button=tk.Button(self,text="Stop Recording",
                                   command=self.stop_recording,state=tk.DISABLED)
        self.stop_button.pack(pady=20)

        self.play_button=tk.Button(self,text="Play Recording",
                                   command=self.play_recording,state=tk.DISABLED)
        self.play_button.pack(pady=20)

        self.convert_button=tk.Button(self,text="Convert to Text",
    ...
    def convert_audio_to_text(self):
        ...
        with sr.AudioFile(self.audio_file_path)as source:
            ...
            try:
                ...
                ds=pickle.load(open('preprocess_data.pkl','rb'))
                if 0<=question_number<len(ds): #question_number>=0 and question_number<len(ds)
